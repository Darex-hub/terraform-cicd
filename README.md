...
The CI workflow runs on pull requests to the main branch and can also be triggered manually.
...
The CD workflow runs on pushes to the main branch and can also be triggered manually.
It performs the following steps:
...
This workflow uses a GitHub environment named dev. Infrastructure is only applied after code is merged into main or when the workflow is triggered manually.
...
CI workflow: Open a pull request to main. The workflow will run automatically and show the Terraform plan.

CD workflow: Merge changes into main or trigger the workflow manually. The workflow will apply infrastructure changes.
